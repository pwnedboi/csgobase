...
void RenderManager::drawBoxOutline(int x, int y, int w, int h, Color color)
{
    ...
    this->drawBox(x - 1, y - 1, w + 2, h + 2, Color(0, 0, 0, alpha)); // The border, black
    this->drawBox(x + 1, y + 1, w - 2, h - 2, Color(0, 0, 0, alpha));
}
...
{
    espfont     = this->createFont("ProggyTiny", 10, FONTFLAG_OUTLINE);
