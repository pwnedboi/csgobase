...
bool ImGui::ButtonEx2(const char* label, bool enabled, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    col = enabled ? GetColorU32(ImGuiCol_ButtonActive) : col;
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::Button(const char* label, bool enabled, const ImVec2& size_arg)
{
    return ButtonEx2(label, enabled, size_arg, 0);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    if(hovered && !held)
    {
        col = GetColorU32(ImVec4(.2, .2, .2, .8));
    }
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, buf, (int)buf_size, ImVec2(-1, 0), flags, callback, user_data);
}
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_ButtonActive);
